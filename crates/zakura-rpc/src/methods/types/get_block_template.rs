...
impl MinerParams {
    ...
    // The `push_value` below `expect`s a type-guaranteed invariant
...
where
    State: Service<
        ...
        + 'static,
{
    let request = zakura_state::ReadRequest::ChainInfo;
    let response = state
    ...
    let chain_info = match response {
        zakura_state::ReadResponse::ChainInfo(chain_info) => chain_info,
