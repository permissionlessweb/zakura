...
fn adjust_difficulty_and_time_for_testnet(
    ...
    // Only offer a minimum-difficulty template once the current time is strictly past the
    // consensus threshold. Switching earlier would clamp the template time into the future,
    // producing unnecessary minimum-difficulty blocks that reduce Testnet difficulty.
...
mod tests {
    ...
    const PREVIOUS_BLOCK_TIME: u32 = 1_600_000_000;
    ...
    fn recent_block_data(network: &Network) -> Vec<(CompactDifficulty, DateTime<Utc>)> {
        ...
        (0..POW_ADJUSTMENT_BLOCK_SPAN)
            .map(|index| {
                let index =
                    u32::try_from(index).expect("the difficulty adjustment block span fits in u32");
                (
                    threshold,
                    DateTime32::from(PREVIOUS_BLOCK_TIME - index).into(),
                )
            })
            .collect()
    }
    ...
    fn chain_info(cur_time: u32) -> GetBlockTemplateChainInfo {
        GetBlockTemplateChainInfo {
            ...
            min_time: DateTime32::from(PREVIOUS_BLOCK_TIME - 100),
            max_time: DateTime32::from(PREVIOUS_BLOCK_TIME + BLOCK_MAX_TIME_SINCE_MEDIAN),
        }
    }

    #[test]
    fn current_time_before_threshold_stays_standard_and_is_not_future_dated() {
        let network = Network::new_default_testnet();
        let cur_time = PREVIOUS_BLOCK_TIME + 400;
        ...
        assert_eq!(
            result.max_time,
            DateTime32::from(PREVIOUS_BLOCK_TIME + MINIMUM_DIFFICULTY_GAP)
        );
    }

    #[test]
    fn current_time_past_threshold_uses_minimum_difficulty() {
        let network = Network::new_default_testnet();
        let cur_time = PREVIOUS_BLOCK_TIME + MINIMUM_DIFFICULTY_GAP + 1;
        let mut result = chain_info(cur_time);
        ...
        assert_eq!(result.cur_time, DateTime32::from(cur_time));
        assert_eq!(result.min_time, DateTime32::from(cur_time));
    }
}
