...
fn highest_verified_anchor(
    ...
    // its replay. Fall back to a higher source when one exists, and otherwise keep reporting the
    // anchor that does not fit rather than replaying from genesis into a check that cannot pass —
    // the caller learns which anchor was in the way instead of which root was missing.
    if anchor > height {
        if best.is_none() {
    ...
    // case below: a memo or grid entry that already covers `height` does not need the band's lower
    // edge.
    if db
