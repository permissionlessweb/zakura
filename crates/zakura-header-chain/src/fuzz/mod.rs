...
impl FuzzStore {
    fn new(mode: EngineMode) -> Self {
        ...
        let context_archive =
            HashMap::from([(frontier.hash, (frontier.height, genesis.header.clone()))]);
        Self {
            graph,
            metadata,
            selected: vec![frontier],
    ...
    fn insertion_with_validation(
        ...
        for offset in 1..=count {
            ...
            let height = parent
                ...
                .0
                .checked_add(offset)
                .expect("the bounded fuzz batch height does not overflow");
            header.time = regtest_genesis_block()
                .header
                .time
                .checked_add_signed(chrono::Duration::seconds(i64::from(height)))
                .expect("the bounded fuzz timestamp remains representable");
...
impl FullStateEvidenceAuthority for FuzzAuthority {
    fn authorizes_full_state(&self, _event: &crate::TransitionEvent) -> bool {
        true
    }
            ...
            4 => {
                ...
                TransitionRequest {
                    ...
                    event: crate::TransitionEvent::OperatorReconsider(OperatorReconsider {
                        ...
                        invalidation_evidence: Some(evidence(operation.saturating_sub(1), branch)),
                        evidence: evidence(operation, branch),
                    }),
                }
            }
...
fn assert_generation_delta(
    ...
    let header_changed =
        finalized_changed || eligibility_changed || selected_path_changed || header_graph_changed;
...
mod tests {
    ...
    #[test]
    // AUD-INCIDENT: this exact event order reproduces the historical stale
    // completion bug and proves the promoted branch remains extendable.
    fn late_completion_cannot_break_promoted_branch() {
    ...
    // A small set of readable examples checks deterministic replay independently
    // of the untracked corpus that libFuzzer builds during a campaign.
    #[test]
    fn bounded_transition_examples_replay_deterministically() {
        let corpus: &[(&str, &[u8])] = &[
            ("linear_growth", &[0, 1, 2, 3, 4, 5, 6, 7]),
            ("fork_replacement", &[0, 1, 2, 8, 9, 11, 12]),
            ("stale_completion", &[0, 16, 17, 18, 19]),
            ("operator_cycle", &[0, 1, 2, 24, 25, 32, 33, 34, 35]),
            ("crash_reopen", &[0, 1, 40, 2, 41, 3, 42]),
            ("body_invalid", &[10, 48]),
            ("body_unavailable", &[10, 56]),
            ("deferred_reevaluation", &[80, 88, 72]),
            ("verified_finality", &[10, 96, 104]),
            ("body_mismatch", &[10, 112]),
            ("evict_pressure", &[120]),
            ("losing_branch_promotion", &[65, 66, 67, 67]),
            ("shorter_higher_work", &[65, 68]),
            ("same_height_permutations", &[84]),
            ("consecutive_resets", &[82]),
            ("late_completion_after_promotion", &[73]),
            ("fixed_anchor_boundaries", &[70]),
            ("block_spec_mutations", &[86]),
            ("body_evidence_matrix", &[89]),
        ];
        for (name, bytes) in corpus {
            let first = replay_fork_transition_bytes(bytes);
            let second = replay_fork_transition_bytes(bytes);
            assert_eq!(first, second, "{name} must replay deterministically");
            if *name == "losing_branch_promotion" {
                assert_eq!(
                    first.snapshot.frontiers.header_best.height,
                    block::Height(6)
                );
            }
            if *name == "shorter_higher_work" {
                assert_eq!(
                    first.snapshot.frontiers.header_best.height,
                    block::Height(2)
                );
            }
            if *name == "same_height_permutations" {
                assert_eq!(first.permutation_checks, 1);
            }
            if *name == "consecutive_resets" {
                assert_eq!(first.reset_checks, 1);
            }
            if *name == "late_completion_after_promotion" {
                assert_eq!(first.incident_checks, 1);
            }
            if *name == "fixed_anchor_boundaries" {
                assert_eq!(first.boundary_checks, 1);
            }
